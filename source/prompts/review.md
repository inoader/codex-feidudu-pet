Create one horizontal animation strip for Codex pet `meituan-feidudu`, state `review`.
...
Specific choreography: remain seated, lean head slightly forward/down, bring one paw thoughtfully under the chin, scan with the eyes and make a small thoughtful head tilt, then return. The other paw remains on belly. Distinct from waiting for a person.
